...
if current_count >= MAX_DAYS:
    print(f"Already have {MAX_DAYS} days of VIX data.")
    conn.close()
    exit()
...
if not new_data:
    print("No more VIX data to fetch.")
    conn.close()
    exit()
...
print(f"Inserted {len(new_data)} new days of VIX data. Total now: {current_count + len(new_data)} days.")
